...
import { Search, Upload, Bell, User, Home, Play, Clock, MoreVertical, Heart, Flame, Settings as SettingsIcon } from 'lucide-react';
...
const SavedVideos = () => {
  ...
  const savedVideos = Array(12).fill().map((_, index) => ({
    id: index + 1,
    title: `Saved Video ${index + 1}`,
    channel: `Channel ${index % 5 + 1}`,
    views: `${Math.floor(Math.random() * 300)}K views`,
    ...
    thumbnail: `https://picsum.photos/320/180?random=${index + 300}`
  }));
  ...
  const menuItems = [
    ...
    { icon: Heart, text: 'Saved videos', path: '/saved-videos', active: true },
