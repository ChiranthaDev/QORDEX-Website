...
const Watchlist = () => {
  ...
  // Mock data for watchlist videos
  const watchlistVideos = Array(15).fill().map((_, index) => ({
    id: index + 1,
    title: `Watchlist Video ${index + 1}`,
    channel: `Channel ${index % 4 + 1}`,
    views: `${Math.floor(Math.random() * 200)}K views`,
    ...
    thumbnail: `https://picsum.photos/320/180?random=${index + 200}`,
    progress: Math.floor(Math.random() * 90) + 10 // Percentage watched
  }));
  ...
  const menuItems = [
    ...
    { icon: Clock, text: 'Watchlist', path: '/watchlist', active: true },
              ...
              {watchlistVideos.map((video) => (
                ...
                >
                  <div className="relative rounded-xl overflow-hidden mb-3">
                    ...
                    {/* Progress percentage */}
                    <div className="absolute top-2 right-2 bg-black bg-opacity-80 text-white text-xs px-1.5 py-0.5 rounded">
                      {video.progress}%
                    </div>
                  </div>
